...
from ..const import DPType as IntegrationDPType

DPType = IntegrationDPType
...
class TuyaBLEDevice:
    ...
    async def _send_datapoints(self, datapoint_ids: list[int]) -> None:
        ...
        if self._protocol_version == 3:
            ...
            _LOGGER.error(
                "%s: Sending datapoints is not supported for protocol version %s",
                self.address,
                self._protocol_version,
            )
            raise TuyaBLEError(
                "Sending datapoints is not supported for protocol version %s"
                % (self._protocol_version)
            )
